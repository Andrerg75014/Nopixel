...
AddEventHandler("robbery:scanLock", function(lockpick,cardType)
    ...
    --  if cardType == "125" then
        ...
        if VaultDoor ~= 0 then
          TriggerServerEvent("robbery:TriggerAlarm", "10-90, An alarm has been triggered at a fleeca Bank")
          TriggerServerEvent('phone:assistRequestSilent', 'AIcall17', 'A.I. Phone Call')
          --print("vault trigger")
...
AddEventHandler("robbery:scanbank", function(bankID,newBanks)
    ...
    if #(vector3(GetEntityCoords(PlayerPedId())) - vector3(banks[bank]["x"],banks[bank]["y"],banks[bank]["z"])) < 10.0 then
