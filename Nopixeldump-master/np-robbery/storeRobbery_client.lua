...
AddEventHandler("lucky:store", function()
	...
	TriggerEvent("chatMessage", "EMAIL ", 8, "Hey, I heard a store has some hot shit in it, go check it out @ " .. SecurityCamLocations[CorrectStore]["info"])
end)
...
AddEventHandler("robbery:register", function()
	...
	TriggerServerEvent('mission:completed', math.random(140,1250))
end)
...
AddEventHandler("robbery:safe", function()
	...
	if tonumber(storeid) == tonumber(CorrectStore) then
		...
		TriggerServerEvent('mission:completed', math.random(1500,4050))
	end

	TriggerServerEvent('mission:completed', math.random(200,1050))
