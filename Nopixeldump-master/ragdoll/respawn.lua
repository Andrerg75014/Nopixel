...
RegisterNetEvent("kill:confirm:now")
AddEventHandler("kill:confirm:now", function()
    if imDead ~= 1 then
        print("Someone thinks I am dead, though, I am not?")
        -- here we kill them if it seems to work
    end
end)
...
AddEventHandler('trycpr', function()
    ...
    if curDist < 5 or curDist2 < 5 then
        local finished = exports["np-taskbar"]:taskBar(120000,"Performing CPR (3p)")
