...
AddEventHandler("COP:PayTow", function()
	if onJob and not recentpay then
		recentpay = true
		TriggerServerEvent("server:givepayJob", "Tow Truck from Police Officer", 500)
		Citizen.Wait(300000)
		recentpay = false
	else
...
AddEventHandler("COP:PayTow", function(amount)
	...
	if amount == nil then
		if not recentpay then
			recentpay = true
			TriggerServerEvent("server:givepayJob", "Government Payment", 500)
			Citizen.Wait(300000)
			recentpay = false
		else
...
AddEventHandler('event:control:towgarage', function(useID)
	...
	if allow then
		...
		if targetVehicle ~= 0 then
			...
			TriggerServerEvent("repo:car",plate,targetVehicle)
...
AddEventHandler('towgarage:craft', function(res,amount)
	...
	if rank >= 4 or rank2 > 1 then -- Eddie wants rank 4 for crafting
		...
		if not res then
			TriggerEvent("customNotification","1 = Tuner (100 Elect), 2 = Plate (3 Copper), 3 = Nitrous (2 Alu), 4 = Lockpick Set(2 Steel), 5 = Adv Repair Kit(3 Electronics)")
			return
		end
...
AddEventHandler('repo:success', function(vehicle)
