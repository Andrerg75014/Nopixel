...
function cmd.Init()
    ...
    Citizen.CreateThread(function()
        local speed = 0.5
        TriggerEvent("admin:isFlying",cmd.vars.enable)
        while true do
            Citizen.Wait(0)
            if cmd.vars.enable then
            ...
            for k,v in pairs(cmd.vars.cloaked) do
                ...
                if not v then
                    ...
                    if ped == PlayerPedId() then
                        ...
                        NetworkFadeOutEntity(ped, false, false)
                    else
...
function cmd.RunCommand(caller, args)
    if not args.message then return end
    local src = caller:getVar("source")
    TriggerClientEvent('player:receiveItem', src, args.message, 1)
    exports["np-log"]:AddLog("Admin", caller, "Spawned item", {item = tostring(args.message)}) 
    NPX.Admin:Log(log, caller)
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.message then if WarMenu.Button("Spawn Item: " .. cmd.vars.message) then NPX.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.Init()
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            for i = 0, 255 do
                if cmd.vars.toggle and NetworkIsPlayerActive(i) and IsPlayerPlaying(i) and not (PlayerPedId() == GetPlayerPed(i)) then
...
cmd = {
    ...
    category = "Player",
    usage = "bCop <source>",
    description = "Attempts to sign onto cop",
    ranks = {"admin"},
    vars = {}
}
function cmd.RunCommand(caller, args)
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Request  " .. cmd.vars.target.name .. " into cop.") then NPX.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, runontarget = true}) end end
end
