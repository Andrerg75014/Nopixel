...
on("__cfx_nui:GiveItem", (data, cb) => {
	...
	emit("hud-display-item",id,"Received")
...
RegisterNetEvent('inv:sendItemList')
on('inv:sendItemList', (itemList) => {
...
on('inventory:removeItem', (id, amount) => {
	...
	emit("hud-display-item",id,"Removed")
})
...
function spam() {
	...
	emit("player:receiveItem","electronics",1)
	emit("player:receiveItem","thermite",1)
	emit("player:receiveItem","fuse",1)
	emit("player:receiveItem","clutch",1)
	emit("player:receiveItem","drillbit",1)
	emit("player:receiveItem","battery",1)
	emit("player:receiveItem","breadboard",1)
	emit("player:receiveItem","screen",1)
	emit("player:receiveItem","heavycutters",1)
	emit("player:receiveItem","cleaninggoods",1)
	emit("player:receiveItem","riflebody",1)
	emit("player:receiveItem","riflebodybox",1)
	emit("player:receiveItem","drill",1)
	emit("player:receiveItem","electronickit",1)
	emit("player:receiveItem","valuablegoods",1)
	emit("player:receiveItem","nitrous",1)
	emit("player:receiveItem","binoculars",1)
	emit("player:receiveItem","evidence",1)
	emit("player:receiveItem","greencow",1)
	emit("player:receiveItem","armor",1)
	emit("player:receiveItem","scanner",1)
	emit("player:receiveItem","advlockpick",1)
	emit("player:receiveItem","inkedmoneybag",1)
	emit("player:receiveItem","ciggy",1)
	emit("player:receiveItem","evidencebag",1)
	emit("player:receiveItem","sealedevidencebag",1)
	emit("player:receiveItem","repairtoolkit",1)
	emit("player:receiveItem","mobilephone",1)
	emit("player:receiveItem","radio",1)
	emit("player:receiveItem","umbrella",1)
...
function BuildInventory(Inventory) {
	...
	for (let i = 0; i < buildInv.length; i++) { 
		invArray[itemCount] = { amount: buildInv[i].amount, item_id: buildInv[i].item_id, id: buildInv[i].id,  name: buildInv[i].name, information: buildInv[i].information,  slot: buildInv[i].slot };
