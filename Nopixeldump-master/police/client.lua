...
local lawSignOn = { -- #MarkedForMarker
	{249.29241943359,-369.28234863281,-44.137638092041},
	{-1579.71,-580.48,108.53}
}

local evidencelocker = { -- #MarkedForMarker
    {483.74,-987.64,24.23},
    {2059.515625,2993.2145996094,-72.702003479004},
    {325.05,-1629.5, -66.78},
}
...
Citizen.CreateThread(function()
	while true do
		...
		for i,v in ipairs(lawSignOn) do
			local distCheck = #(vector3(v[1],v[2],v[3]) - GetEntityCoords(PlayerPedId()))
			if distCheck < 2 then
				if signedonBar == 1 then
					DrawText3DTest(v[1],v[2],v[3],"~r~E~w~ Sign Off law work.")
				else
					DrawText3DTest(v[1],v[2],v[3],"~r~E~w~ Sign On law work.")
				end
	          shouldWait = false
	...
	while true do
		...
		if isCop then
			...
			for i,v in ipairs(evidencelocker) do
				dstCheck = #(vector3(v[1],v[2],v[3]) - GetEntityCoords(PlayerPedId()))
				if dstCheck < 30 then nearby = true end
				if dstCheck < 0.7 then
				 	DrawText3DTest(v[1],v[2],v[3],"~r~E~w~ Open Evidence Locker")
				end
			...
			dstCheck = #(vector3(486.04,-988.9,24.23) - GetEntityCoords(PlayerPedId()))
			if dstCheck < 30 then nearby = true end
			if dstCheck < 2 then
			 	DrawText3DTest(486.04,-988.9,24.23,"~r~E~w~ Open Trash Locker.")
			end



			dstCheck = #(vector3(1840.45, 2572.82, 46.02) - GetEntityCoords(PlayerPedId()) )
			if dstCheck < 30 then nearby = true end
			if dstCheck < 2 then
			 	DrawText3DTest(1840.45, 2572.82, 46.02,"~r~E~w~ Open Trash Locker.")
			end		
...
function DrawMarkerRad(markerType,x,y,z,arg1,arg2,arg3,arg4,arg5,arg6,sx,sy,sz,r,g,b,a,upDown,face,p19,rotate,texture,textureName,drawEnts)
    ...
    if dist < 30 then
      local dist2 = math.floor(a - (dist * 25))
      if dist2 < 0 then dist2 = 0 end
      DrawMarker(markerType,x,y,z,arg1,arg2,arg3,arg4,arg5,arg6,sx,sy,sz,r,g,b,dist2,upDown,face,p19,rotate,texture,textureName,drawEnts)
    end
end
...
Citizen.CreateThread( function()
	...
	while true do 
		...
		if recentsearch == 0 then
			endDistance = #(vector3(2132.8930664063,2919.697265625,-61.796829223633) - GetEntityCoords(PlayerPedId()))
...
Citizen.CreateThread( function()
	...
	while true do 
		Citizen.Wait(600)
		local pos = GetOffsetFromEntityInWorldCoords(GetPlayerPed(PlayerId()), 0.0, 0.0, 0.0)
		if GetNumberOfFiresInRange(pos,1.7) > 1 and hasBeenOnFire < 4 then
			hasBeenOnFire = hasBeenOnFire + 1
			StartEntityFire(PlayerPedId())
			local health = (GetEntityHealth(PlayerPedId()) - 25)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			local finished = exports["np-taskbar"]:taskBar(2000,"Unracking Shotgun")
		  	if finished == 100 then
				TriggerServerEvent("actionclose", PlayerPedId(), "Shotgun", PlayerId())
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			local finished = exports["np-taskbar"]:taskBar(2000,"Unracking Rifle")
		  	if finished == 100 then
				TriggerServerEvent("actionclose", PlayerPedId(), "Carbine Rifle", PlayerId())
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	if #(GetEntityCoords( PlayerPedId()) - GetEntityCoords(cuffPed)) < 1.5 then
		handcuffType = 16
		handCuffed = true
		handCuffedWalking = false
		TriggerEvent("police:currentHandCuffedState",handCuffed,handCuffedWalking)
...
AddEventHandler('police:tenThirteen', function()
    ...
    local pos = GetEntityCoords(PlayerPedId(),  true)
	  ...
	  if street[2] ~= 0 and street[2] ~= nil then
	    message = string.format("10-13 (Officer Down) at %s, nearby %s", GetStreetNameFromHashKey(street[1]), GetStreetNameFromHashKey(street[2]))
	  else
	    message = string.format("10-13 (Officer Down) at %s", GetStreetNameFromHashKey(street[1]))
	  end
		TriggerServerEvent('police:tenThirteenAlert', message)
	end
end)
...
AddEventHandler("police:tenForteen", function()	
		...
		local street = table.pack(GetStreetNameAtCoord(pos.x, pos.y, pos.z))
		if street[2] ~= 0 and street[2] ~= nil then
			message = string.format("10-14 (Medic Down) at %s, nearby %s", GetStreetNameFromHashKey(street[1]), GetStreetNameFromHashKey(street[2]))
		else
			message = string.format("10-14 (Medic Down) at %s", GetStreetNameFromHashKey(street[1]))
		end
		TriggerServerEvent('police:tenThirteenAlert', message)
end)
...
AddEventHandler("K9:Huntfind", function()
	...
	players, distances, coords = GetClosestPlayers()
	if(#players > 0) then
...
AddEventHandler('dr:drag', function(pl)
	otherid = tonumber(pl)
	print("attempting to be dragged")
...
local function serviceVehicle(arg, livery, isEmsWhiteListed, cb)
	...
	if isCop then
		...
		selectedSkin = copVehicleList[arg]

	else
		if isEmsWhiteListed then
			if arg > #emsVehicleListWhite then arg = 1 end
			selectedSkin = emsVehicleListWhite[arg]
		else
