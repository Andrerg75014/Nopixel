...
function bleedMulti(WeaponValue)
    ...
    if tier1[""..part..""] then
        SetPedToRagdoll(PlayerPedId(), math.random(1000,2000), math.random(1000,2000), 2, 0, 0, 0)
        Wait(2000)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if GetEntityHealth(PlayerPedId()) < lasthealth and GetPedArmour(PlayerPedId()) < 1 then
        	...
        	if lasthealth - GetEntityHealth(PlayerPedId()) > 30 and not bleeddamage and not bleeddamage2 then
	        	SetPedToRagdoll(PlayerPedId(), math.random(500), math.random(500), 2, 0, 0, 0)
	        end
        	lasthealth = GetEntityHealth(PlayerPedId())
...
AddEventHandler('eventConc', function(level)
	...
	if math.random(100) > 40 then
		...
		if not fullcol then
			...
			SetPedToRagdoll(PlayerPedId(), levelset, levelset, 0, 0, 0, 0) 
...
function MovementImpair(impairLevel,impairType)
	...
	if impairType == 4 then
		...
		if math.random( fct ) > 170 then
  
			SetPedToRagdoll(PlayerPedId(), fct, fct, 3, 0, 0, 0) 
			Citizen.Wait(fct * 10)
		end
	...
	for o = 1, #bones do
		...
		if bones[o]["boneIndex"] == part and ( (lasthealth2 > GetEntityHealth(PlayerPedId()) and ( lasthealth2 - GetEntityHealth(PlayerPedId())) > 1 ) or lastarmor > GetPedArmour(PlayerPedId()) ) then
            ...
            if allowdamage then
                ...
                if bones[o]["injuryType"] == 2 then
                    SetPedToRagdoll(PlayerPedId(), math.random(1000,4000), math.random(1000,4000), 2, 0, 0, 0)
                end
