...
function pickUpCash()
    ...
    if not pickingup then
        TriggerServerEvent('phone:assistRequestSilent', 'AIcall27', 'A.I. Phone Call')
        local coords = GetEntityCoords(GetPlayerPed(-1))
       -- Citizen.Trace("Doing Animation")
        ...
        while pickingup do
            ...
            if math.random(200) > 195 then
                TriggerEvent("player:receiveItem","decrypterenzo",math.random(3))
                TriggerEvent("player:receiveItem","markedbills",math.random(100))
            end

            if chance < 10 then
                TriggerServerEvent('phone:assistRequestSilent', 'AIcall27', 'A.I. Phone Call')
                 TriggerEvent("player:receiveItem","markedbills",math.random(10))
            end

            TriggerServerEvent( 'mission:completed',math.random(200) )
