...
function AreaChecked()
    ...
    if gangSpots[Area] then
    	if gangSpots[Area]["zone"] ~= gangType then
    		print("This is not my hood")
    		allowGunChance = true
	    	return true
	    else
	    	print("This is my hood")
	...
	if searchedpreviously then
		...
		if math.random(100) > 60 then
			if math.random(100) > 85 then
				TriggerEvent("payment:chopshopscrap",1,false)
			end
...
AddEventHandler('housing:info', function(args)
	...
	TriggerEvent("DoLongHudText","Looking up Information on " .. house_name,99)
...
AddEventHandler('housing:findsalecid', function(checkcid,price,house_id,house_model,name,x,y,z)
	...
	while buying > 0 do
		...
		if name ~= nil then
			TriggerEvent("DoLongHudText","Offered property " .. housename .. " with down payment of $3000 then $3000 per week, Type /confirm to confirm.",91)
		else
	...
	if buy then
		...
		if name ~= nil then
			...
			TriggerServerEvent("house:purchasehouse",house_id,house_model,upfront,housename,false) 
		end
	end
...
AddEventHandler('GarageData', function(garages)
	...
	for i = 1, #garages do
		...
		local house_owner_cid = tonumber(garages[i]["house_owner_cid"]) 
		if house_model < 3 or house_model == 5 then
			...
			mygarages["garage"..table_id] = { ["house_owner_cid"] = house_owner_cid, ["house_name"] = house_name, ["house_id"] = house_id, ["house_model"] = house_model, ["house_poi"] = garages[i]["house_poi"],  ["x"] = x,  ["y"] = y,  ["z"] = z,  ["table_id"] = garages[i]["table_id"] }
		end
	end
...
AddEventHandler('housing:info:realtor', function(args)
	if modifying then
		...
		if args == "setclothing" then
			...
			TriggerEvent("DoLongHudText","A new clothing area has been set, there is " .. #modifyingTable["clothing"] .. " in total!",91)
			return
		end
		...
		if args == "setstorage" then
			...
			TriggerEvent("DoLongHudText","A new storage area has been set, there is " .. #modifyingTable["storage"] .. " in total!",91)
			return
		end
...
function DropItemPed2()
	...
	if math.random(100) > 95 then
		TriggerEvent('player:receiveItem',453432689,1, true)
	end

	if math.random(100) > 95 then
		TriggerEvent('player:receiveItem',1593441988,1, true)
	end

	if math.random(100) > 95 then
		TriggerEvent('player:receiveItem',137902532,1, true)
	end
...
function DropItemPed()
    ...
    if math.random(100) > 92 and not itemrec then
		TriggerEvent("payment:chopshopscrap",2,false)
	end	
end
...
Citizen.CreateThread(function()
 	...
 	while true do
		...
		local dstOfficeExit = #(plyCoords - vector3(158.05, -20.06, 35.45))

		if dstOffice < 3.0 then
			DrawText3Ds( 172.78, -26.89, 68.35, '[E] Enter Job Centre' )
		elseif dstOfficeExit < 3.0 then
			DrawText3Ds( 158.05, -20.06, 35.45, '[E] Exit Job Centre' )
		else
...
AddEventHandler("weed:startcropInsideCheck", function(weedType)
	if entryType == 2 then
		print("We can plant here, we are in a player house!")
...
function buildNorth()
	...
	local building = CreateObject(`V_16_Studio_LoShell`,generator.x+1.05453500,generator.y-3.61683300,generator.z+0.64563890,false,false,false)
	FreezeEntityPosition(building,true)
	SetEntityHeading(PlayerPedId(),0.0)
...
function buildTrevor()
	...
	local building = CreateObject(`V_61_shell_walls`,generator.x+0.42222900,generator.y-0.13131700,generator.z+1.18855100,false,false,false)
	FreezeEntityPosition(building,true)

	CreateObject(`V_61_bd1_mesh_delta`,generator.x+6.58658700,generator.y+1.90211400,generator.z+1.40669800,false,false,false)
	CreateObject(`V_61_bd1_over_normal`,generator.x+6.90869900,generator.y+1.90231200,generator.z+1.41739500,false,false,false)

	CreateObject(`V_Res_Tre_lightFan`,generator.x+6.92140500,generator.y+1.75621500,generator.z+3.91411600,false,false,false)

	CreateObject(`V_61_bd2_mesh_delta`,generator.x+7.45918600,generator.y-2.23859200,generator.z+1.40669800,false,false,false)
	CreateObject(`V_61_bed2_over_normal`,generator.x+7.39488900,generator.y-2.23812300,generator.z+1.41739900,false,false,false)
	CreateObject(`V_Res_Tre_lightFan`,generator.x+7.43066600,generator.y-2.13224100,generator.z+3.91411600,false,false,false)



	CreateObject(`V_61_bd1_mesh_curtains`,generator.x+9.47294700,generator.y+2.01390900,generator.z+1.40529700,false,false,false)
	CreateObject(`V_61_bd2_mesh_curtains`,generator.x+7.45918600,generator.y-2.23859100,generator.z+1.40669800,false,false,false)

	CreateObject(`V_61_bth_mesh_delta`,generator.x+2.90302400,generator.y-3.25595700,generator.z+1.40669900,false,false,false)
	CreateObject(`V_61_bth_mesh_window`,generator.x+2.54372400,generator.y-4.20030900,generator.z+2.59330000,false,false,false)
	CreateObject(`V_61_bth_mesh_sink`,generator.x+3.63269400,generator.y-3.89074800,generator.z+1.44331700,false,false,false)
	CreateObject(`V_61_bth_mesh_bath`,generator.x+4.62711300,generator.y-3.17999400,generator.z+1.40002500,false,false,false)
	CreateObject(`V_61_bth_mesh_mirror`,generator.x+3.63412700,generator.y-4.18290000,generator.z+1.69656600,false,false,false)

	CreateObject(`v_serv_bs_cond`,generator.x+4.96474600,generator.y-4.15317100,generator.z+1.27547500,false,false,false)
	CreateObject(`v_ret_ps_toiletry_02`,generator.x+5.04444800,generator.y-4.10563800,generator.z+1.18004900,false,false,false)
	CreateObject(`v_ret_ps_toiletry_01`,generator.x+5.05485300,generator.y-2.21447100,generator.z+0.63780390,false,false,false)

	CreateObject(`V_Res_Tre_lightFan`,generator.x-1.78290200,generator.y+1.75875700,generator.z+3.91411600,false,false,false)
	CreateObject(`V_Res_Tre_lightFan`,generator.x-6.62357700,generator.y+1.83180300,generator.z+3.91411600,false,false,false)
	CreateObject(`V_Res_Tre_lightFan`,generator.x-6.62351200,generator.y-1.99185700,generator.z+3.91411600,false,false,false)
	CreateObject(`V_61_lng_over_normal`,generator.x-4.57882400,generator.y-0.13147000,generator.z+1.55739500,false,false,false)
	CreateObject(`V_61_lng_over_decal`,generator.x-4.53859100,generator.y-0.13131600,generator.z+1.40719200,false,false,false)
	CreateObject(`V_61_lng_mesh_delta`,generator.x-4.52318900,generator.y-0.12544300,generator.z+1.40691200,false,false,false)


	CreateObject(`V_61_lng_mesh_curtains`,generator.x-8.64869900,generator.y-0.14242800,generator.z+1.39529700,false,false,false)


	CreateObject(`V_61_lng_mesh_windows2`,generator.x-1.81416800,generator.y+3.99466500,generator.z+1.91970800,false,false,false)
	CreateObject(`V_61_hlw_mesh_delta`,generator.x+2.25451900,generator.y-0.11426500,generator.z+1.40669900,false,false,false)

	CreateObject(`V_61_hlw_over_normals`,generator.x+2.29985700,generator.y-1.19603700,generator.z+1.41739400,false,false,false)
	CreateObject(`V_Res_Tre_lightFan`,generator.x+2.31997200,generator.y-0.08653700,generator.z+3.91411600,false,false,false)

	CreateObject(`V_61_fnt_mesh_delta`,generator.x+0.60459100,generator.y-3.17859000,generator.z+1.40669800,false,false,false)
	CreateObject(`V_61_fnt_over_normal`,generator.x+0.60557090,generator.y-3.17862900,generator.z+1.41739300,false,false,false)
	CreateObject(`V_61_fdr_over_decal`,generator.x+2.27650100,generator.y-1.13863200,generator.z+1.40569900,false,false,false)


	CreateObject(`V_61_ktm_mesh_delta`,generator.x-2.52016200,generator.y-2.09838000,generator.z+1.40605500,false,false,false)

	CreateObject(`V_61_ktn_mesh_windows`,generator.x-2.28900200,generator.y-4.23605800,generator.z+2.6737700,false,false,false)
	CreateObject(`V_61_ktn_mesh_delta`,generator.x-2.51330800,generator.y-2.07973900,generator.z+1.40688100,false,false,false)
	CreateObject(`V_61_ktn_over_normal`,generator.x-4.57882400,generator.y-0.13147000,generator.z+1.52539500,false,false,false)
	CreateObject(`V_61_ktn_mesh_lights`,generator.x-2.62422800,generator.y-2.19258700,generator.z+4.10724400,false,false,false)

	CreateObject(`V_61_shell_doorFrames`,generator.x+5.16617300,generator.y-1.16292300,generator.z+1.40882800,false,false,false)
	CreateObject(`V_61_shell_FdFrame`,generator.x+0.61957400,generator.y-4.27097800,generator.z+1.39039500,false,false,false)

	CreateObject(`V_61_Bath_over_dec`,generator.x+2.66080000,generator.y-3.60888800,generator.z+0.01784800,false,false,false)
	SetEntityHeading(PlayerPedId(),267.0)
	SetGameplayCamRelativeHeading(0.0)
...
function buildApartment(heading)
	...
	local building = CreateObject(`v_49_motelmp_shell_2`,generator.x,generator.y,generator.z,false,false,false)
...
function buildHouse()
	...
	local building = CreateObject(`v_16_mid_shell_2`,generator.x+2.29760700,generator.y-1.33191200,generator.z+1.26253700,false,false,false)
	FreezeEntityPosition(building,true)
	Citizen.Wait(100)
...
robberyExitCoords = {
	...
	[44] = { ['x'] = 1404.48, ['y'] = -563.109, ['z'] = 74.4965, ['h'] = 294.0 },
