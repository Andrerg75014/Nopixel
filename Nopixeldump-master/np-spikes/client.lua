...
AddEventHandler('spikes:watchtarget', function(watchsent)
	...
	while true do
	    ...
	    if not IsPedInAnyVehicle(ped, false) then
	    	print("no longer watching, not in vehicle"  .. watching)
	    	return
	    end
	    if driverPed ~= PlayerPedId() then 
	    	print("not the driver now, removing spikes"  .. watching)
	    	return
	    end
		if spikeCoords[watching] == nil then
			print("removed one because it was no longer in the table"  .. watching)
			return
		end
		...
		if not spikeCoords[watching]["watching"] then
			print("Table refreshed, removing spike" .. watching)
			return
		end
