...
function doRefreshAllPlayerData(serverData)
	for playerServerId, playerData in pairs(serverData) do
		for key, data in pairs(playerData) do
			if playersData[playerServerId] == nil then playersData[playerServerId] = {} end
			if playersData[playerServerId][key] == nil then playersData[playerServerId][key] = {} end
...
function draw3dtext(text, posX, posY, posZ, r, g, b, a)
	...
	local dist = #(localPos - vector3(posX, posY, posZ));
	local maxDist = 100;
	local size = 0.2;
...
function table.tostring(tbl)
	...
	for k, v in ipairs(tbl) do
		result[#result+1]= table.val_to_str(v);
		done[k] = true;
	end
	for k, v in pairs(tbl) do
		if (not done[k]) then
			result[#result+1]= table.key_to_str(k).."="..table.val_to_str(v);
		end
	end
