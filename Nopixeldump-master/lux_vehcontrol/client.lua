...
Controlkey = {["vehicleSlights"] = {85,"Q"},["vehicleSsound"] = {19,"LEFTALT"},["vehicleSnavigate"] = {80,"R"}} 
RegisterNetEvent('event:control:update')
AddEventHandler('event:control:update', function(table)
	Controlkey["vehicleSlights"] = table["vehicleSlights"]
	Controlkey["vehicleSsound"] = table["vehicleSsound"]
	Controlkey["vehicleSnavigate"] = table["vehicleSnavigate"]
end)
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						if not IsPauseMenuActive() then
							...
							-- TOG DFLT SRN LIGHTS
							if IsDisabledControlJustReleased(0, Controlkey["vehicleSlights"][1])  then 
							...
							-- TOG LX SIREN
							elseif IsDisabledControlJustReleased(0, Controlkey["vehicleSsound"][1]) then
							...
							if state_lxsiren[veh] > 0 then
								if IsDisabledControlJustReleased(0, Controlkey["vehicleSnavigate"][1]) then
									if IsVehicleSirenOn(veh) then
										local cstate = state_lxsiren[veh]
										local nstate = 1
										PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1) -- on
							...
							if state_lxsiren[veh] < 1 then
								if IsDisabledControlPressed(0, Controlkey["vehicleSnavigate"][1]) then -- nfi same as above
									actv_manu = true
								else
									actv_manu = false
								end
