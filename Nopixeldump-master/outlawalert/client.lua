...
AddEventHandler('outlawNotifyID', function(player, info)
    ...
    if sonid == monid then
        TriggerEvent('chatMessage', "", {30, 144, 255}, "^5ID: " .. info.first .. " | " .. info.last)
        TriggerEvent('chatMessage', "", {30, 144, 255}, "#cref661241-" .. info.cid .. " | #pref81902-" .. player) 
        TriggerEvent('chatMessage', "", {30, 144, 255}, " Sex: " .. info.sex .." | Born: " .. info.dob)
        ...
        for i,v in ipairs(gasStations) do
            ...
            if scandst < 10 and scandst < dstcheck then
                ...
                if IsExplosionInSphere(9,v[1],v[2],v[3],60.0)  then
                    ...
                    TriggerServerEvent('phone:assistRequestSilent', 'AIcall23', 'A.I. Phone Call')
                    if s2 == 0 then
                        TriggerServerEvent('ExplosionInProgress1', street1)
                    elseif s2 ~= 0 then
                        TriggerServerEvent("ExplosionInProgress", street1, street2)
                    end
                    Wait(9000)
                    origin3 = false
                end
