...
local pillWorker = { ['x'] = 67.63,['y'] = -1570.74,['z'] = -2.82,['h'] = 268.58, ['info'] = ' lol' }
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if HasStolenGoods() then
			...
			if math.random(200) == 69 then
				...
				TriggerEvent( "player:receiveItem", "locksystem", math.random(2) )
			end
			...
			if OxyRun then
				cashPayment = math.random(150,170)
				if math.random(100) > 45 then
					TriggerEvent( "player:receiveItem", "oxy", math.random(2) )
				end

			else
				cashPayment = math.random(100,180)
			end
...
function checkCars()
	for i = 1, #vehicleList do
		...
		if not IsModelAVehicle(hash) then print(vehicleList[i]["modelname"] .. " failed") end
	end
end
...
AddEventHandler('payment:chopshopscrap', function(rarity,playsound)
	...
	local tradingTable = { "scrapmetal","electronics","plastic","glass","rubber","copper","aluminium","steel" }
	if playsound then
	...
	for i=1,amount do
		for i,v in ipairs(tradingTable) do
			...
			if (v == 26) then
				TriggerEvent('player:receiveItem', v, 3)
			end
			if random > 50 then 
				TriggerEvent('player:receiveItem', v, math.random(3))	
				Citizen.Wait(250)
			end
		end
	end
    ...
    while true do
	  ...
	  --  local dropOff3 = #(vector3(GetEntityCoords(PlayerPedId())) - vector3(chopinfo[3]["x"],chopinfo[3]["y"],chopinfo[3]["z"]))
		...
		if dropOff4 < 1.5 then
			DrawText3Ds(pillStore["x"]-1.4,pillStore["y"]-4.47,pillStore["z"]-32.2, "[E] to Leave") 
			if IsControlJustReleased(0,38) then
				CleanUpArea()
